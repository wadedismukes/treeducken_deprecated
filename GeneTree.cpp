...
void GeneTree::initializeTree(std::set<int> extantLociInd, std::multimap<int,double> locusDeathMap){
    if(!(nodes.empty())){
        nodes.clear();
    }
    ...
    Node *p;
    for(std::set<int>::iterator it = extantLociInd.begin(); it != extantLociInd.end(); ++it){
...
double GeneTree::getTimeToNextEvent(){
    double ct;
    ct = -log(rando->uniformRv()) / (popSize);
    
    return ct;
}


void GeneTree::censorCoalescentProcess(double stopTime, int contempSpeciesIndx, int ancSpIndx){
    int indx;
    Node *p;
    ...
    time = coalescingNodes[0]->getDeathTime();
    it = coalescingNodes.begin();
    
    while(coalescingNodes.size() > 1){
        time -= getTimeToNextEvent();
        if(time < stopTime){
            time = stopTime;
            break;
        }
